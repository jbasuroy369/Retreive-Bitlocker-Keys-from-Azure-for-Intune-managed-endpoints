...
.SYNOPSIS
Script to retrieve bitlocker keys for devices against deviceName provided as a CSV input. Needs to be executed as Entra registered application
...
.INPUTS
A CSV file having device names for which you want to export the bitlocker keys. See required parameters

.OUTPUTS
Retrieves bitlocker keys for specified devices and exports result to csv file per device.

.DESCRIPTION
Script to retrieve bitlocker keys for devices against deviceName provided as a CSV input and export results to csv file per device. 
...
foreach($device in $csvData){
    ...
    $azDevice = Get-MgDevice -Filter "displayName eq '$($device.DeviceName)'" -ExpandProperty registeredOwners
